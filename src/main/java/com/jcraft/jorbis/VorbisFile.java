...
public final class VorbisFile implements AutoCloseable {
    ...
    private boolean decode_ready = false;
...
//            is = new SeekableInputStream(file);
    ...
    public VorbisFile(ByteBuffer is, byte[] initial, int ibytes)
    ...
    private int get_data() {
        ...
        if (remainingChunk >= CHUNKSIZE) {
            ...
            System.arraycopy(datasource.array(), index, buffer, 0, remainingChunk);
            datasource.position(datasource.capacity());
            oy.wrote(remainingChunk);
    ...
    private int bisect_forward_serialno(long begin, long searched, long end,
            int currentNo, int m) {
...
//        serialNos = new int[links];
    ...
    @Deprecated
    private int open_seekable() throws JOrbisException {
...
//        return 0;
    ...
    int open_callbacks(ByteBuffer is, byte[] initial, int ibytes//, callbacks callbacks
    ) throws JOrbisException {
        int ret;
        datasource = is;
...
//        }
    ...
    private int read(byte[] buffer, int length, int bigendianp, int word, int sgned,
            int[] bitstream) {
...
//        datasource.close();
    ...
    /** @deprecated only useful for on disk files. */
    @Deprecated
