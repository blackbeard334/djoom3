...
public class tr_turboshadow {
    ...
    public static srfTriangles_s R_CreateVertexProgramTurboShadowVolume(final idRenderEntityLocal ent, final srfTriangles_s tri, final idRenderLightLocal light, srfCullInfo_t cullInfo) {
        ...
        if (USE_TRI_DATA_ALLOCATOR) {
            ...
            tempIndexes = newTri.getIndexes();
            shadowIndexes = newTri.getIndexes();
        } else {
        ...
        newTri.setNumIndexes(newTri.numShadowIndexesNoFrontCaps = numShadowIndexes + (numShadowingFaces * 6));
        ...
        if (USE_TRI_DATA_ALLOCATOR) {
            ...
            R_ResizeStaticTriSurfIndexes(newTri, newTri.getNumIndexes());
        } else {
            // allocate memory for the indexes
            R_AllocStaticTriSurfIndexes(newTri, newTri.getNumIndexes());
            // copy the indexes we created for the sil planes
            SIMDProcessor.Memcpy(newTri.getIndexes(), tempIndexes, numShadowIndexes /* sizeof( tempIndexes[0] )*/);
        }
