...
public class tr_trace {
    ...
    public static localTrace_t R_LocalTrace(final idVec3 start, final idVec3 end, final float radius, final srfTriangles_s tri) {
        ...
        for (i = 0, j = 0; i < tri.numIndexes; i += 3, j++) {
            ...
            triOr = cullBits[tri.indexes.getIntBuffer().get(i + 0)];
            triOr |= cullBits[tri.indexes.getIntBuffer().get(i + 1)];
            triOr |= cullBits[tri.indexes.getIntBuffer().get(i + 2)];
            ...
            dir[0] = tri.verts[tri.indexes.getIntBuffer().get(i + 0)].xyz.oMinus(point);
            dir[1] = tri.verts[tri.indexes.getIntBuffer().get(i + 1)].xyz.oMinus(point);
            ...
            if (d > 0.0f) {
                ...
                edge = tri.verts[tri.indexes.getIntBuffer().get(i + 0)].xyz.oMinus(tri.verts[tri.indexes.getIntBuffer().get(i + 1)].xyz);
                ...
                if (d < 0.0f) {
                    edge = tri.verts[tri.indexes.getIntBuffer().get(i + 0)].xyz.oMinus(tri.verts[tri.indexes.getIntBuffer().get(i + 2)].xyz);
                    d = dir[0].oMultiply(edge);
                    if (d < 0.0f) {
                    ...
                    edge = tri.verts[tri.indexes.getIntBuffer().get(i + 1)].xyz.oMinus(tri.verts[tri.indexes.getIntBuffer().get(i + 2)].xyz);
            ...
            dir[2] = tri.verts[tri.indexes.getIntBuffer().get(i + 2)].xyz.oMinus(point);
            ...
            if (d > 0.0f) {
                ...
                d = dir[1].oMultiply(edge);
                if (d < 0.0f) {
                    edge = tri.verts[tri.indexes.getIntBuffer().get(i + 1)].xyz.oMinus(tri.verts[tri.indexes.getIntBuffer().get(i + 0)].xyz);
                    d = dir[1].oMultiply(edge);
                    if (d < 0.0f) {
                        if (dir[1].LengthSqr() > radiusSqr) {
                            continue;
                    ...
                    edge = tri.verts[tri.indexes.getIntBuffer().get(i + 2)].xyz.oMinus(tri.verts[tri.indexes.getIntBuffer().get(i + 0)].xyz);
            ...
            if (d > 0.0f) {
                ...
                d = dir[2].oMultiply(edge);
                if (d < 0.0f) {
                    edge = tri.verts[tri.indexes.getIntBuffer().get(i + 2)].xyz.oMinus(tri.verts[tri.indexes.getIntBuffer().get(i + 1)].xyz);
                    d = dir[2].oMultiply(edge);
                    if (d < 0.0f) {
                        if (dir[2].LengthSqr() > radiusSqr) {
                            continue;
            ...
            hit.point = point;
            hit.indexes[0] = tri.indexes.getIntBuffer().get(i);
            hit.indexes[1] = tri.indexes.getIntBuffer().get(i + 1);
            hit.indexes[2] = tri.indexes.getIntBuffer().get(i + 2);
        }
    ...
    public static void RB_DrawExpandedTriangles(final srfTriangles_s tri, final float radius, final idVec3 vieworg) {
        ...
        for (i = 0; i < tri.numIndexes; i += 3) {
            ...
            final idVec3[] p/*[3]*/ = {
                        tri.verts[tri.indexes.getIntBuffer().get(i + 0)].xyz,
                        tri.verts[tri.indexes.getIntBuffer().get(i + 1)].xyz,
                        tri.verts[tri.indexes.getIntBuffer().get(i + 2)].xyz};
