...
//        R_StaticFree(depthReadback);
            ...
            for (i = 0; i < 2; i++) {
                ...
                for (; surf != null; surf = surf.nextOnLight) {
                    ...
                    for (int j = 0; j < tri.getNumIndexes(); j += 3) {
                        final int i1 = tri.getIndexes()[j + 0];
                        final int i2 = tri.getIndexes()[j + 1];
                        final int i3 = tri.getIndexes()[j + 2];
            ...
            for (i = 0; i < 2; i++) {
                for (surf = (i != 0 ? vLight.localShadows[0] : vLight.globalShadows[0]); surf != null; surf = surf.nextOnLight) {
                    ...
                    if (r_showShadowCount.GetInteger() == 3) {
                        ...
                        if (tri.numShadowIndexesNoCaps != tri.getNumIndexes()) {
                            continue;
                        }
                    }
    ...
    public static void RB_T_RenderTriangleSurfaceAsLines(final drawSurf_s surf) {
        ...
        qglBegin(GL_LINES);
        for (int i = 0; i < tri.getNumIndexes(); i += 3) {
            for (int j = 0; j < 3; j++) {
                final int k = (j + 1) % 3;
...
//            System.out.println("vModels.modelViewMatrix="+vModels.modelViewMatrix[0]);
    ...
    public static void RB_ShowTexturePolarity(drawSurf_s[] drawSurfs, int numDrawSurfs) {
        ...
        for (i = 0; i < numDrawSurfs; i++) {
            ...
            for (j = 0; j < tri.getNumIndexes(); j += 3) {
                idDrawVert a, b, c;
                final float[] d0 = new float[5], d1 = new float[5];
                float area;

                a = tri.verts[tri.getIndexes()[j]];
                b = tri.verts[tri.getIndexes()[j + 1]];
                c = tri.verts[tri.getIndexes()[j + 2]];
            ...
            for (i = 0; i < numDrawSurfs; i++) {
                ...
                for (j = 0; j < tri.getNumIndexes(); j += 3) {
                    pos = R_LocalPointToGlobal(drawSurf.space.modelMatrix,
                            (tri.verts[ tri.getIndexes()[ j + 0]].xyz.oPlus(tri.verts[ tri.getIndexes()[ j + 1]].xyz.oPlus(tri.verts[ tri.getIndexes()[ j + 2]].xyz))).oMultiply(1.0f / 3.0f).oPlus(tri.verts[ tri.getIndexes()[ j + 0]].normal.oMultiply(0.2f)));
    ...
    public static void RB_AltShowNormals(drawSurf_s[] drawSurfs, int numDrawSurfs) {
        ...
        for (i = 0; i < numDrawSurfs; i++) {
            ...
            for (j = 0; j < tri.getNumIndexes(); j += 3) {
                ...
                v[0] = tri.verts[tri.getIndexes()[j + 0]];
                v[1] = tri.verts[tri.getIndexes()[j + 1]];
                v[2] = tri.verts[tri.getIndexes()[j + 2]];
...
//            if (i != 101) continue;
            ...
            for (j = 0; j < tri.getNumIndexes(); j += 3) {
                ...
                a = tri.verts[tri.getIndexes()[j + 0]];
                b = tri.verts[tri.getIndexes()[j + 1]];
                c = tri.verts[tri.getIndexes()[j + 2]];
        ...
        for (i = 0; i < numDrawSurfs; i++) {
            ...
            for (j = 0; j < tri.getNumIndexes(); j += 3) {
                for (k = 0; k < 3; k++) {
                    int l, i1, i2;
                    l = (k == 2) ? 0 : k + 1;
                    i1 = tri.getIndexes()[j + k];
                    i2 = tri.getIndexes()[j + l];

                    // if these are used backwards, the edge is shared
                    for (m = 0; m < tri.getNumIndexes(); m += 3) {
                        for (n = 0; n < 3; n++) {
                            o = (n == 2) ? 0 : n + 1;
                            if ((tri.getIndexes()[m + n] == i2) && (tri.getIndexes()[m + o] == i1)) {
                                break;
                            }
                        }
                        if (n != 3) {
            ...
            danglePlane = tri.getNumIndexes() / 3;
