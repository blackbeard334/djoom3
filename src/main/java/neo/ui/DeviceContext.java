...
public class DeviceContext {
    ...
    public static class idDeviceContext {
        ...
        public void Init() {
            ...
            this.whiteImage.SetSort(SS_GUI);
            this.mbcs = false;
            SetupFonts();
            this.activeFont = fonts.oGet(0);
            ...
            this.cursorImages[CURSOR_ARROW.ordinal()] = declManager.FindMaterial("ui/assets/guicursor_arrow.tga");
            this.cursorImages[CURSOR_HAND.ordinal()] = declManager.FindMaterial("ui/assets/guicursor_hand.tga");
            this.scrollBarImages[SCROLLBAR_HBACK.ordinal()] = declManager.FindMaterial("ui/assets/scrollbarh.tga");
            this.scrollBarImages[SCROLLBAR_VBACK.ordinal()] = declManager.FindMaterial("ui/assets/scrollbarv.tga");
            this.scrollBarImages[SCROLLBAR_THUMB.ordinal()] = declManager.FindMaterial("ui/assets/scrollbar_thumb.tga");
            this.scrollBarImages[SCROLLBAR_RIGHT.ordinal()] = declManager.FindMaterial("ui/assets/scrollbar_right.tga");
            this.scrollBarImages[SCROLLBAR_LEFT.ordinal()] = declManager.FindMaterial("ui/assets/scrollbar_left.tga");
            this.scrollBarImages[SCROLLBAR_UP.ordinal()] = declManager.FindMaterial("ui/assets/scrollbar_up.tga");
            this.scrollBarImages[SCROLLBAR_DOWN.ordinal()] = declManager.FindMaterial("ui/assets/scrollbar_down.tga");
            this.cursorImages[CURSOR_ARROW.ordinal()].SetSort(SS_GUI);
            this.cursorImages[CURSOR_HAND.ordinal()].SetSort(SS_GUI);
            this.scrollBarImages[SCROLLBAR_HBACK.ordinal()].SetSort(SS_GUI);
            this.scrollBarImages[SCROLLBAR_VBACK.ordinal()].SetSort(SS_GUI);
            this.scrollBarImages[SCROLLBAR_THUMB.ordinal()].SetSort(SS_GUI);
            this.scrollBarImages[SCROLLBAR_RIGHT.ordinal()].SetSort(SS_GUI);
            this.scrollBarImages[SCROLLBAR_LEFT.ordinal()].SetSort(SS_GUI);
            this.scrollBarImages[SCROLLBAR_UP.ordinal()].SetSort(SS_GUI);
            this.scrollBarImages[SCROLLBAR_DOWN.ordinal()].SetSort(SS_GUI);
...
//
        ...
        public void DrawMaterial(float x, float y, float w, float h, final idMaterial mat, final idVec4 color, float scaleX /*= 1.0f*/, float scaleY /*= 1.0f*/) {
            ...
            final float[] s0 = {0}, s1 = {0}, t0 = {0}, t1 = {0};
            final float[] x1 = {x}, y1 = {y}, w1 = {w}, h1 = {h};
// 
...
// 
        ...
        public void DrawRect(float x, float y, float width, float height, float size, final idVec4 color) {
            ...
            final float[] x1 = {x}, y1 = {y}, w1 = {width}, h1 = {height};
            ...
            AdjustCoords(x1, y1, w1, h1);
            DrawStretchPic(x1[0], y1[0], size, h1[0], 0, 0, 0, 0, this.whiteImage);
            DrawStretchPic((x1[0] + w1[0]) - size, y1[0], size, h1[0], 0, 0, 0, 0, this.whiteImage);
            DrawStretchPic(x1[0], y1[0], w1[0], size, 0, 0, 0, 0, this.whiteImage);
            DrawStretchPic(x1[0], (y1[0] + h1[0]) - size, w1[0], size, 0, 0, 0, 0, this.whiteImage);
        }
        ...
        public void DrawFilledRect(float x, float y, float width, float height, final idVec4 color) {
            ...
            AdjustCoords(x1, y1, w1, h1);
            DrawStretchPic(x1[0], y1[0], w1[0], h1[0], 0, 0, 0, 0, this.whiteImage);
            aaaa++;
        }
        static int aaaa = 0;
        ...
        public int DrawText(String text, float textScale, int textAlign, idVec4 color, idRectangle rectDraw, boolean wrap, int cursor /*= -1*/, boolean calcOnly /*= false*/, idList<Integer> breaks /*= NULL*/, int limit /*= 0*/) {
            ...
            final float charSkip = MaxCharWidth(textScale) + 1;
            final float lineSkip = MaxCharHeight(textScale);

            final float cursorSkip = (cursor >= 0 ? charSkip : 0);
...
//	newLinePtr = NULL;
            ...
            while (p_i < text.length()) {
                ...
                if ((buff[len - 1] != C_COLOR_ESCAPE) && ((len <= 1) || (buff[len - 2] != C_COLOR_ESCAPE))) {
                    textWidth += textScale * this.useFont.glyphScale * this.useFont.glyphs[ buff[len - 1]].xSkip;
        ...
        public void DrawStretchPic(float x, float y, float w, float h, float s0, float t0, float s1, float t1, final idMaterial shader) {
            ...
            final boolean identity = !this.mat.IsIdentity();
            if (identity) {
                verts[0].xyz.oMinSet(this.origin);
                verts[0].xyz.oMulSet(this.mat);
                verts[0].xyz.oPluSet(this.origin);
                verts[1].xyz.oMinSet(this.origin);
                verts[1].xyz.oMulSet(this.mat);
                verts[1].xyz.oPluSet(this.origin);
                verts[2].xyz.oMinSet(this.origin);
                verts[2].xyz.oMulSet(this.mat);
                verts[2].xyz.oPluSet(this.origin);
                verts[3].xyz.oMinSet(this.origin);
                verts[3].xyz.oMulSet(this.mat);
                verts[3].xyz.oPluSet(this.origin);
            }
        ...
        public void DrawMaterialRotated(float x, float y, float w, float h, final idMaterial mat, final idVec4 color, float scalex /*= 1.0*/, float scaley /*= 1.0*/, float angle /*= 0.0f*/) {
            ...
            final float[] s0 = new float[1], s1 = new float[1], t0 = new float[1], t1 = new float[1];
            final float[] x1 = {x}, y1 = {y}, w1 = {w}, h1 = {h};
            // 
            //  handle negative scales as well	
            if (scalex < 0) {
        ...
        public void DrawStretchPicRotated(float x, float y, float w, float h, float s0, float t0, float s1, float t1, final idMaterial shader, float angle /*= 0.0f*/) {
            ...
            final boolean ident = !this.mat.IsIdentity();
            if (ident) {
                verts[0].xyz.oMinSet(this.origin);
                verts[0].xyz.oMulSet(this.mat);
                verts[0].xyz.oPluSet(this.origin);
        ...
        public int CharWidth(final char c, float scale) {
            ...
            final fontInfo_t font = this.useFont;
        ...
        public int TextWidth(final String text, float scale, int limit) {
            ...
            SetFontByScale(scale);
            final glyphInfo_t[] glyphs = this.useFont.glyphs;
...
//                        continue;
        ...
        public int MaxCharHeight(float scale) {
            SetFontByScale(scale);
            final float useScale = scale * this.useFont.glyphScale;
        ...
        public int FindFont(final String name) {
            ...
            fileName.Replace("fonts", va("fonts/%s", this.fontLang));
...
// }
        ...
        public void SetSize(float width, float height) {
            ...
            if ((width != 0) && (height != 0)) {
                this.xScale = this.vidWidth * (1.0f / width);
                this.yScale = this.vidHeight * (1.0f / height);
            }
        }
...
//}
        ...
        public void DrawCursor(float[] x, float[] y, float size) {
            ...
            if (y[0] >= this.vidHeight) {
                y[0] = this.vidHeight;
            }

            renderSystem.SetColor(colorWhite);
            AdjustCoords(x, y, s, s);
            DrawStretchPic(x[0], y[0], s[0], s[0], 0, 0, 1, 1, this.cursorImages[this.cursor.ordinal()]);
        }

        public void SetCursor(int n) {
            this.cursor = ((n < CURSOR_ARROW.ordinal()) || (n >= CURSOR_COUNT.ordinal())) ? CURSOR_ARROW : CURSOR.values()[n];
        }
        ...
        public boolean ClippedCoords(float[] x, float[] y, float[] w, float[] h, float[] s1, float[] t1, float[] s2, float[] t2) {
            ...
            while (--c > 0) {
                final idRectangle clipRect = this.clipRects.oGet(c);
        ...
        public void PushClipRect(float x, float y, float w, float h) {
            this.clipRects.Append(new idRectangle(x, y, w, h));
        }
        ...
        public void SetOverStrike(boolean b) {
            this.overStrikeMode = b;
        }

        public boolean GetOverStrike() {
            return this.overStrikeMode;
        }
        ...
        public void DrawEditCursor(float x, float y, float scale) {
            ...
            SetFontByScale(scale);
            final float useScale = scale * this.useFont.glyphScale;
            final glyphInfo_t glyph2 = this.useFont.glyphs[(this.overStrikeMode) ? '_' : '|'];
            final float yadj = useScale * glyph2.top;
...
//		memcpy(newColor[0], color[0], sizeof(idVec4));
                ...
                while ((s_i < len) && ((s = text.charAt(s_i)) != 0) && (count < len)) {
                    ...
                    glyph = this.useFont.glyphs[s = text.charAt(s_i)];
                    ...
                    if (idStr.IsColor(ctos(s))) {
                        ...
                        d2++;
                        final float yadj = useScale * glyph.top;
        ...
        private void PaintChar(float x, float y, float width, float height, float scale, float s, float t, float s2, float t2, final idMaterial hShader) {
            final float[] w = {width * scale}, h = {height * scale};
            final float[] x1 = {x}, y1 = {y};
            final float[] s1 = {s}, t1 = {t};
            final float[] s3 = {s2}, t3 = {t2};
