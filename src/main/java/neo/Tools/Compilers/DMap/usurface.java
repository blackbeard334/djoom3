            ...
            for (int eNum = 1; eNum < dmapGlobals.num_entities; eNum++) {
                ...
                for (i = 0; i < model.NumSurfaces(); i++) {
                    ...
                    for (int j = 0; j < tri2.numIndexes; j += 3) {
                        for (int k = 0; k < 3; k++) {
                            final idVec3 v = tri2.verts[tri2.indexes.getIntBuffer().get(j + k)].xyz;

                            mapTri.v[k].xyz = v.oMultiply(axis).oPlus(origin);

                            mapTri.v[k].normal = tri2.verts[tri2.indexes.getIntBuffer().get(j + k)].normal.oMultiply(axis);
                            mapTri.v[k].st = tri2.verts[tri2.indexes.getIntBuffer().get(j + k)].st;
                        }
