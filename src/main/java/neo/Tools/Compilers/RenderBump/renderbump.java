...
//        memset(hash.binLinks, -1, sizeof(hash.binLinks));
        ...
        for (i = 0; i < highMesh.numIndexes; i += 3) {
            ...
            triBounds.Clear();
            for (j = 0; j < 3; j++) {
                triBounds.AddPoint(highMesh.verts[ highMesh.indexes.getIntBuffer().get(i + j)].xyz);
            }
            for (j = 0; j < 3; j++) {
    ...
    static float TraceToMeshFace(final srfTriangles_s highMesh, int faceNum, float minDist, float maxDist,
        ...
        v[0] = highMesh.verts[ highMesh.indexes.getIntBuffer().get( (faceNum * 3) + 0)].xyz;
        v[1] = highMesh.verts[ highMesh.indexes.getIntBuffer().get( (faceNum * 3) + 1)].xyz;
        v[2] = highMesh.verts[ highMesh.indexes.getIntBuffer().get( (faceNum * 3) + 2)].xyz;
        ...
        for (j = 0; j < 3; j++) {
            sampledNormal.oPluSet(highMesh.verts[ highMesh.indexes.getIntBuffer().get( (faceNum * 3) + j)].normal.oMultiply(bary[j]));
        }
        ...
        for (int i = 0; i < 4; i++) {
            float color = 0.0f;
            for (j = 0; j < 3; j++) {
                color += bary[j] * highMesh.verts[ highMesh.indexes.getIntBuffer().get( (faceNum * 3) + j)].color[i];
            }
    ...
    static void RasterizeTriangle(final srfTriangles_s lowMesh, final idVec3[] lowMeshNormals, int lowFaceNum, renderBump_t[] rbs) {
        ...
        verts[0][0] = (lowMesh.verts[ lowMesh.indexes.getIntBuffer().get((lowFaceNum * 3) + 0)].st.oGet(0) * rbs[0].width) - 0.5f;
        verts[1][0] = (lowMesh.verts[ lowMesh.indexes.getIntBuffer().get((lowFaceNum * 3) + 1)].st.oGet(0) * rbs[0].width) - 0.5f;
        verts[2][0] = (lowMesh.verts[ lowMesh.indexes.getIntBuffer().get((lowFaceNum * 3) + 2)].st.oGet(0) * rbs[0].width) - 0.5f;
        verts[0][1] = (lowMesh.verts[ lowMesh.indexes.getIntBuffer().get((lowFaceNum * 3) + 0)].st.oGet(1) * rbs[0].width) - 0.5f;
        verts[1][1] = (lowMesh.verts[ lowMesh.indexes.getIntBuffer().get((lowFaceNum * 3) + 1)].st.oGet(1) * rbs[0].width) - 0.5f;
        verts[2][1] = (lowMesh.verts[ lowMesh.indexes.getIntBuffer().get((lowFaceNum * 3) + 2)].st.oGet(1) * rbs[0].width) - 0.5f;
...
//                float[] edgeDistance = rb.edgeDistances[k / 4];
                ...
                for (k = 0; k < 3; k++) {
                    ...
                    index = lowMesh.indexes.getIntBuffer().get((lowFaceNum * 3) + k);
...
//			}
    ...
    static idRenderModel CombineModelSurfaces(idRenderModel model) {
        ...
        final int[]/*glIndex_t*/ indexes = newTri.indexes.getAsIntArray();
        numIndexes = 0;
        numVerts = 0;
        for (i = 0; i < model.NumSurfaces(); i++) {
            final modelSurface_s surf = model.Surface(i);
...
//            memcpy(verts + numVerts, tri.verts, tri.numVerts * sizeof(tri.verts[0]));
            ...
            for (j = 0; j < tri.numIndexes; j++) {
                indexes[numIndexes + j] = numVerts + tri.indexes.getIntBuffer().get(j);
            }
                        ...
                        if (colorPass != 0) {
                            ...
                            for (j = 0; j < mesh.numIndexes; j += 3) {
                                for (k = 0; k < 3; k++) {
                                    ...
                                    v = mesh.indexes.getIntBuffer().get(j + k);
                            ...
                            for (j = 0; j < mesh.numIndexes; j += 3) {
                                if (flat) {
                                    ...
                                    v1 = mesh.indexes.getIntBuffer().get(j + 0);
                                    v2 = mesh.indexes.getIntBuffer().get(j + 1);
                                    v3 = mesh.indexes.getIntBuffer().get(j + 2);
