...
//                    str.Clear();
                ...
                if (token.subtype != subtype) {
                    this.Error("expected '%s' but found '%s'", scriptstack.GetPunctuationFromId(subtype), token.getData());
                    return false;
                }
            }
            return true;
...
//            Mem_Free(define);
        ...
        private boolean Directive_include() throws Lib.idException {
            ...
            if (token.type == TT_STRING) {
                ...
                path.oPluSet("/");
                path.oPluSet(token);
                if (!script.LoadFile(path.getData(), OSPath)) {
                    // try absolute path
                    path.oSet(token);
                    if (!script.LoadFile(path.getData(), OSPath)) {
...
//					delete script;
                ...
                script = new idLexer();
                if (!script.LoadFile(includepath.oPlus(path).getData(), OSPath)) {
//			delete script;
                    script = null;
                }
            } else {
