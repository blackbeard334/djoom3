...
public class Frustum {
    ...
    public static class idFrustum {
        ...
        public void MoveFarDistance(float dFar) {
            ...
            final float scale = dFar / this.dFar;
            this.dFar = dFar;
            this.dLeft *= scale;
            this.dUp *= scale;
            this.invFar = 1.0f / dFar;
        ...
        public idVec3 GetCenter() {						// returns center of frustum
            return this.origin.oPlus(this.axis.oGet(0).oMultiply((this.dFar - this.dNear) * 0.5f));
        }

        public boolean IsValid() {							// returns true if the frustum is valid
            return (this.dFar > this.dNear);
        }
...
//
        ...
        public idFrustum Expand(final float d) {					// returns frustum expanded in all directions with the given value
            final idFrustum f = new idFrustum(this);//TODO:oSET
            f.origin.oMinSet(f.axis.oGet(0).oMultiply(d));
            f.dFar += 2.0f * d;
            f.dLeft = f.dFar * this.dLeft * this.invFar;
            f.dUp = f.dFar * this.dUp * this.invFar;
            f.invFar = 1.0f / this.dFar;
            return f;
        }

        public idFrustum ExpandSelf(final float d) {					// expands frustum in all directions with the given value
            this.origin.oMinSet(this.axis.oGet(0).oMultiply(d));
            this.dFar += 2.0f * d;
            this.dLeft = this.dFar * this.dLeft * this.invFar;
            this.dUp = this.dFar * this.dUp * this.invFar;
            this.invFar = 1.0f / this.dFar;
            return this;
        }

        public idFrustum Translate(final idVec3 translation) {	// returns translated frustum
            final idFrustum f = new idFrustum(this);
...
//
        ...
        public float PlaneDistance(final idPlane plane) {
            final float[] min = new float[1], max = new float[1];
...
//
        ...
        public boolean CullPoint(final idVec3 point) {
            ...
            p = (point.oMinus(this.origin)).oMultiply(this.axis.Transpose());
            // test whether or not the point is within the frustum
            if ((p.x < this.dNear) || (p.x > this.dFar)) {
                return true;
            }
            scale = p.x * this.invFar;
            if (idMath.Fabs(p.y) > (this.dLeft * scale)) {
                return true;
            }
            if (idMath.Fabs(p.z) > (this.dUp * scale)) {
                return true;
            }
            return false;
        }
        ...
        public boolean CullBounds(final idBounds bounds) {
            ...
            localOrigin = (center.oMinus(this.origin)).oMultiply(this.axis.Transpose());
        ...
        public boolean CullBox(final idBox box) {
            ...
            localOrigin = (box.GetCenter().oMinus(this.origin)).oMultiply(this.axis.Transpose());
            localAxis = box.GetAxis().oMultiply(this.axis.Transpose());
        ...
        public boolean CullSphere(final idSphere sphere) {
            ...
            center = (sphere.GetOrigin().oMinus(this.origin)).oMultiply(this.axis.Transpose());
            r = sphere.GetRadius();

            // test near plane
            if ((this.dNear - center.x) > r) {
                return true;
            }

            // test far plane
            if ((center.x - this.dFar) > r) {
                return true;
            }

            rs = r * r;
            sFar = this.dFar * this.dFar;

            // test left/right planes
            d = (this.dFar * idMath.Fabs(center.y)) - (this.dLeft * center.x);
            if ((d * d) > (rs * (sFar + (this.dLeft * this.dLeft)))) {
                return true;
            }

            // test up/down planes
            d = (this.dFar * idMath.Fabs(center.z)) - (this.dUp * center.x);
            if ((d * d) > (rs * (sFar + (this.dUp * this.dUp)))) {
                return true;
            }
        ...
        public boolean CullFrustum(final idFrustum frustum) {
            ...
            final idVec3[] indexPoints = new idVec3[8], cornerVecs = new idVec3[4];

            // transform the given frustum into the space of this frustum
            localFrustum = new idFrustum(frustum);
            localFrustum.origin = (frustum.origin.oMinus(this.origin)).oMultiply(this.axis.Transpose());
            localFrustum.axis = frustum.axis.oMultiply(this.axis.Transpose());
        ...
        public boolean CullWinding(final idWinding winding) {
            ...
            for (i = 0; i < winding.GetNumPoints(); i++) {
                localPoints[i] = (winding.oGet(i).ToVec3().oMinus(this.origin)).oMultiply(transpose);
            }
...
//
        ...
        public boolean IntersectsBox(final idBox box) {
            ...
            localFrustum.origin = (this.origin.oMinus(box.GetCenter())).oMultiply(box.GetAxis().Transpose());
            localFrustum.axis = this.axis.oMultiply(box.GetAxis().Transpose());
        ...
        public boolean IntersectsSphere(final idSphere sphere) {
            ...
            p = (sphere.GetOrigin().oMinus(this.origin)).oMultiply(this.axis.Transpose());

            if (p.x <= this.dNear) {
                scale = this.dNear * this.invFar;
                dir.y = idMath.Fabs(p.y) - (this.dLeft * scale);
                dir.z = idMath.Fabs(p.z) - (this.dUp * scale);
            } else if (p.x >= this.dFar) {
                dir.y = idMath.Fabs(p.y) - this.dLeft;
                dir.z = idMath.Fabs(p.z) - this.dUp;
            } else {
                scale = p.x * this.invFar;
                dir.y = idMath.Fabs(p.y) - (this.dLeft * scale);
                dir.z = idMath.Fabs(p.z) - (this.dUp * scale);
            ...
            if (p.x < this.dNear) {
                scale = this.dLeft * this.dNear * this.invFar;
                if (p.x < (this.dNear + ((scale - p.y) * scale * this.invFar))) {
                    scale = this.dUp * this.dNear * this.invFar;
                    if (p.x < (this.dNear + ((scale - p.z) * scale * this.invFar))) {
                        x = 1;
                    }
                }
            } else {
                if (p.x > this.dFar) {
                    x = 2;
                } else if (p.x > (this.dFar + ((this.dLeft - p.y) * this.dLeft * this.invFar))) {
                    x = 2;
                } else if (p.x > (this.dFar + ((this.dUp - p.z) * this.dUp * this.invFar))) {
                    x = 2;
                }
            }
            ...
            switch (index) {
                ...
                case VORONOI_INDEX_1_0_0:
                    return ((this.dNear - p.x) < r);
                case VORONOI_INDEX_2_0_0:
                    return ((p.x - this.dFar) < r);
                case VORONOI_INDEX_0_1_0:
                    d = (this.dFar * p.y) - (this.dLeft * p.x);
                    return ((d * d) < (r * r * ((this.dFar * this.dFar) + (this.dLeft * this.dLeft))));
                case VORONOI_INDEX_0_2_0:
                    d = (-this.dFar * p.z) - (this.dLeft * p.x);
                    return ((d * d) < (r * r * ((this.dFar * this.dFar) + (this.dLeft * this.dLeft))));
                case VORONOI_INDEX_0_0_1:
                    d = (this.dFar * p.z) - (this.dUp * p.x);
                    return ((d * d) < (r * r * ((this.dFar * this.dFar) + (this.dUp * this.dUp))));
        ...
        public boolean IntersectsFrustum(final idFrustum frustum) {
            final idVec3[] indexPoints2 = new idVec3[8], cornerVecs2 = new idVec3[4];
            idFrustum localFrustum2;

            localFrustum2 = new idFrustum(frustum);
            localFrustum2.origin = (frustum.origin.oMinus(this.origin)).oMultiply(this.axis.Transpose());
            localFrustum2.axis = frustum.axis.oMultiply(this.axis.Transpose());
            ...
            final idVec3[] indexPoints1 = new idVec3[8], cornerVecs1 = new idVec3[4];
            idFrustum localFrustum1;

            localFrustum1 = new idFrustum(this);//TODO:SET
            localFrustum1.origin = (this.origin.oMinus(frustum.origin)).oMultiply(frustum.axis.Transpose());
            localFrustum1.axis = this.axis.oMultiply(frustum.axis.Transpose());
        ...
        public boolean LineIntersection(final idVec3 start, final idVec3 end) {
            return LocalLineIntersection((start.oMinus(this.origin)).oMultiply(this.axis.Transpose()), (end.oMinus(this.origin)).oMultiply(this.axis.Transpose()));
        }
        ...
        public boolean RayIntersection(final idVec3 start, final idVec3 dir, float[] scale1, float[] scale2) {
            if (LocalRayIntersection((start.oMinus(this.origin)).oMultiply(this.axis.Transpose()), dir.oMultiply(this.axis.Transpose()), scale1, scale2)) {//TODO:scale back ref??
                return true;
            }
...
//#if 1
            ...
            for (j = 0; j < 2; j++) {
                ...
                this.axis.oSet(0, dir);
                this.axis.oSet(1, box.GetAxis().oGet(bestAxis).oMinus(this.axis.oGet(0).oMultiply(box.GetAxis().oGet(bestAxis).oMultiply(this.axis.oGet(0)))));
                this.axis.oGet(1).Normalize();
                this.axis.oGet(2).Cross(this.axis.oGet(0), this.axis.oGet(1));

                BoxToPoints(
                        (box.GetCenter().oMinus(projectionOrigin)).oMultiply(this.axis.Transpose()),
                        box.GetExtents(),
                        box.GetAxis().oMultiply(this.axis.Transpose()),
                        points);
                ...
                if (j == 0) {
                    dir.oPluSet(this.axis.oGet(1).oMultiply(idMath.Tan16(0.5f * (idMath.ATan16(points[minY].y, points[minY].x) + idMath.ATan16(points[maxY].y, points[maxY].x)))));
                    dir.oPluSet(this.axis.oGet(2).oMultiply(idMath.Tan16(0.5f * (idMath.ATan16(points[minZ].z, points[minZ].x) + idMath.ATan16(points[maxZ].z, points[maxZ].x)))));
                    dir.Normalize();
                }
            }
...
//#endif
        ...
        public boolean ConstrainToBounds(final idBounds bounds) {
            ...
            bounds.AxisProjection(this.axis.oGet(0), min, max);
            newdFar = max[0] - this.origin.oMultiply(this.axis.oGet(0));
            if (newdFar <= this.dNear) {
                MoveFarDistance(this.dNear + 1.0f);
                return false;
            }
        ...
        public boolean ConstrainToBox(final idBox box) {
            ...
            box.AxisProjection(this.axis.oGet(0), min, max);
            newdFar = max[0] - this.origin.oMultiply(this.axis.oGet(0));
            if (newdFar <= this.dNear) {
                MoveFarDistance(this.dNear + 1.0f);
                return false;
        ...
        public boolean ConstrainToSphere(final idSphere sphere) {
            ...
            sphere.AxisProjection(this.axis.oGet(0), min, max);
            newdFar = max[0] - this.origin.oMultiply(this.axis.oGet(0));
            if (newdFar <= this.dNear) {
                MoveFarDistance(this.dNear + 1.0f);
                return false;
        ...
        public boolean ConstrainToFrustum(final idFrustum frustum) {
            ...
            frustum.AxisProjection(this.axis.oGet(0), min, max);
            newdFar = max[0] - this.origin.oMultiply(this.axis.oGet(0));
            if (newdFar <= this.dNear) {
                MoveFarDistance(this.dNear + 1.0f);
                return false;
...
//
        ...
        public void ToPlanes(idPlane planes[]) {			// planes point outwards
            int i;
            final idVec3[] scaled = new idVec3[2];
            final idVec3[] points = new idVec3[4];

            planes[0].oNorSet(this.axis.oGet(0).oNegative());
            planes[0].SetDist(-this.dNear);
            planes[1].oNorSet(this.axis.oGet(0));
            planes[1].SetDist(this.dFar);

            scaled[0] = this.axis.oGet(1).oMultiply(this.dLeft);
            scaled[1] = this.axis.oGet(2).oMultiply(this.dUp);
        ...
        public void ToPoints(idVec3 points[]) {				// 8 corners of the frustum
            final idMat3 scaled = new idMat3();

            scaled.oSet(0, this.origin.oPlus(this.axis.oGet(0).oMultiply(this.dNear)));
            scaled.oSet(1, this.axis.oGet(1).oMultiply(this.dLeft * this.dNear * this.invFar));
            scaled.oSet(2, this.axis.oGet(2).oMultiply(this.dUp * this.dNear * this.invFar));
            ...
            scaled.oSet(0, this.origin.oPlus(this.axis.oGet(0).oMultiply(this.dFar)));
            scaled.oSet(1, this.axis.oGet(1).oMultiply(this.dLeft));
            scaled.oSet(2, this.axis.oGet(2).oMultiply(this.dUp));
...
//
        ...
        public boolean ProjectionBounds(final idBox box, idBounds projectionBounds) {
            ...
            final int[][] pointCull = new int[8][1];
            final float[] scale1 = new float[1], scale2 = new float[1];
            final idFrustum localFrustum;
            final idVec3[] points = new idVec3[8];
            idVec3 localOrigin;
            idMat3 localAxis, localScaled;
            final idBounds bounds = new idBounds(box.GetExtents().oNegative(), box.GetExtents());

            // if the frustum origin is inside the bounds
            if (bounds.ContainsPoint((this.origin.oMinus(box.GetCenter())).oMultiply(box.GetAxis().Transpose()))) {
                // bounds that cover the whole frustum
                final float[] boxMin = new float[1], boxMax = new float[1];
                float base;

                base = this.origin.oMultiply(this.axis.oGet(0));
                box.AxisProjection(this.axis.oGet(0), boxMin, boxMax);
            ...
            if ((outside != 1) && (outside != 2) && (outside != 4) && (outside != 8)) {
                ...
                localOrigin = (this.origin.oMinus(box.GetCenter())).oMultiply(box.GetAxis().Transpose());
                localScaled = this.axis.oMultiply(box.GetAxis().Transpose());
                localScaled.oGet(0).oMulSet(this.dFar);
                localScaled.oGet(1).oMulSet(this.dLeft);
                localScaled.oGet(2).oMulSet(this.dUp);
                ...
                if (((outside & 2) == 2) && ((outside & 8) == 8)) {
                    ...
                    if ((scale1[0] <= scale2[0]) && (scale1[0] >= 0.0f)) {
                        projectionBounds.AddPoint(new idVec3(scale1[0] * this.dFar, -1.0f, -1.0f));
                        projectionBounds.AddPoint(new idVec3(scale2[0] * this.dFar, -1.0f, -1.0f));
                    }
                }
        ...
        public boolean ProjectionBounds(final idSphere sphere, idBounds projectionBounds) {
            ...
            projectionBounds.oGet(1).x = this.dFar;
        ...
        public boolean ProjectionBounds(final idFrustum frustum, idBounds projectionBounds) {
            ...
            if (frustum.ContainsPoint(this.origin)) {
                // bounds that cover the whole frustum
                final float[] frustumMin = new float[1], frustumMax = new float[1];
                float base;

                base = this.origin.oMultiply(this.axis.oGet(0));
                frustum.AxisProjection(this.axis.oGet(0), frustumMin, frustumMax);
            ...
            if ((outside != 1) && (outside != 2) && (outside != 4) && (outside != 8)) {
                ...
                localOrigin = (this.origin.oMinus(frustum.origin)).oMultiply(frustum.axis.Transpose());
                localScaled = this.axis.oMultiply(frustum.axis.Transpose());
                localScaled.oGet(0).oMulSet(this.dFar);
                localScaled.oGet(1).oMulSet(this.dLeft);
                localScaled.oGet(2).oMulSet(this.dUp);
        ...
        public boolean ProjectionBounds(final idWinding winding, idBounds projectionBounds) {
            ...
            transpose = this.axis.Transpose();
            for (i = 0; i < winding.GetNumPoints(); i++) {
                localPoints[i] = (winding.oGet(0).ToVec3().oMinus(this.origin)).oMultiply(transpose);
            }
            ...
            if ((outside != 1) && (outside != 2) && (outside != 4) && (outside != 8)) {
                ...
                winding.GetPlane(plane);
                scaled.oSet(0, this.axis.oGet(0).oMultiply(this.dFar));
                scaled.oSet(1, this.axis.oGet(1).oMultiply(this.dLeft));
                scaled.oSet(2, this.axis.oGet(2).oMultiply(this.dUp));

                // test the outer edges of this frustum for intersection with the winding
                if (((outside & 2) == 2) && ((outside & 8) == 8)) {
                    if (winding.RayIntersection(plane, this.origin, scaled.oGet(0).oMinus(scaled.oGet(1)).oMinus(scaled.oGet(2)), scale)) {
                        projectionBounds.AddPoint(new idVec3(scale[0] * this.dFar, -1.0f, -1.0f));
                    }
                }
                if (((outside & 2) == 2) && ((outside & 4) == 4)) {
                    if (winding.RayIntersection(plane, this.origin, scaled.oGet(0).oMinus(scaled.oGet(1)).oPlus(scaled.oGet(2)), scale)) {
                        projectionBounds.AddPoint(new idVec3(scale[0] * this.dFar, -1.0f, 1.0f));
                    }
                }
                if (((outside & 1) == 1) && ((outside & 8) == 8)) {
                    if (winding.RayIntersection(plane, this.origin, scaled.oGet(0).oPlus(scaled.oGet(1)).oMinus(scaled.oGet(2)), scale)) {
                        projectionBounds.AddPoint(new idVec3(scale[0] * this.dFar, 1.0f, -1.0f));
                    }
                }
                if (((outside & 1) == 1) && ((outside & 2) == 2)) {
                    if (winding.RayIntersection(plane, this.origin, scaled.oGet(0).oPlus(scaled.oGet(1)).oPlus(scaled.oGet(2)), scale)) {
                        projectionBounds.AddPoint(new idVec3(scale[0] * this.dFar, 1.0f, 1.0f));
                    }
                }
            }
        ...
        public boolean ClippedProjectionBounds(final idFrustum frustum, final idBox clipBox, idBounds projectionBounds) {
            ...
            final int[] clipPointCull = new int[8], clipPlanes = new int[4];
            final int[] pointCull = new int[2], startClip = {0}, endClip = {0}, boxPointCull = new int[8];
            float leftScale, upScale;
            final float[] s1 = {0}, s2 = {0}, t1 = {0}, t2 = {0};
            final float[] clipFractions = new float[4];
            idFrustum localFrustum;
            idVec3 localOrigin1, localOrigin2;
			final idVec3 start = new idVec3(), end = new idVec3();
            final idVec3[] clipPoints = new idVec3[8], localPoints1 = new idVec3[8], localPoints2 = new idVec3[8];
            ...
            if (frustum.ContainsPoint(this.origin)) {
                // bounds that cover the whole frustum
                final float[] clipBoxMin = {0}, clipBoxMax = {0}, frustumMin = {0}, frustumMax = {0}, base = {0};

                base[0] = this.origin.oMultiply(this.axis.oGet(0));
                clipBox.AxisProjection(this.axis.oGet(0), clipBoxMin, clipBoxMax);
                frustum.AxisProjection(this.axis.oGet(0), frustumMin, frustumMax);
            ...
            transpose = new idMat3(this.axis);
            transpose.TransposeSelf();
            localFrustum = new idFrustum(frustum);
            localFrustum.origin = (frustum.origin.oMinus(this.origin)).oMultiply(transpose);
            ...
            for (i = 0; i < 4; i++) {
                p1 = i;
                p2 = 4 + i;
                final int[] clipPointCull_p1 = {0}, clipPointCull_p2 = {0};
            ...
            if (!((farCull != 0) && (0 == (nearCull & farCull)))
                ...
                transpose = new idMat3(this.axis);
                transpose.TransposeSelf();
                localOrigin2 = (clipBox.GetCenter().oMinus(this.origin)).oMultiply(transpose);
            ...
            if ((outside != 1) && (outside != 2) && (outside != 4) && (outside != 8)) {
                ...
                transpose = new idMat3(frustum.axis);
                transpose.TransposeSelf();
                localOrigin1 = (this.origin.oMinus(frustum.origin)).oMultiply(transpose);
                localAxis1 = this.axis.oMultiply(transpose);
                localAxis1.oGet(0).oMulSet(this.dFar);
                localAxis1.oGet(1).oMulSet(this.dLeft);
                localAxis1.oGet(2).oMulSet(this.dUp);

                // transform this frustum into the space of the clip bounds
                transpose = new idMat3(clipBox.GetAxis());
                transpose.TransposeSelf();
                localOrigin2 = (this.origin.oMinus(clipBox.GetCenter())).oMultiply(transpose);
                localAxis2 = this.axis.oMultiply(transpose);
                localAxis2.oGet(0).oMulSet(this.dFar);
                localAxis2.oGet(1).oMulSet(this.dLeft);
                localAxis2.oGet(2).oMulSet(this.dUp);
                ...
                if (((outside & 2) != 0) && ((outside & 8) != 0)) {
                    ...
                    if ((s1[0] <= s2[0]) && (s1[0] >= 0.0f)) {
                        ...
                        if ((t1[0] <= t2[0]) && (t2[0] > s1[0]) && (t1[0] < s2[0])) {
                            projectionBounds.AddPoint(new idVec3(s1[0] * this.dFar, -1.0f, -1.0f));
                            projectionBounds.AddPoint(new idVec3(s2[0] * this.dFar, -1.0f, -1.0f));
                        }
                    }
                }
        ...
        private boolean CullLocalBox(final idVec3 localOrigin, final idVec3 extents, final idMat3 localAxis) {
            ...
            // test left/right planes
            d1 = (this.dFar * testOrigin.y) - (this.dLeft * testOrigin.x);
            d2 = idMath.Fabs(extents.oGet(0) * ((this.dFar * testAxis.oGet(0).oGet(1)) - (this.dLeft * testAxis.oGet(0).oGet(0))))
                    + idMath.Fabs(extents.oGet(1) * ((this.dFar * testAxis.oGet(1).oGet(1)) - (this.dLeft * testAxis.oGet(1).oGet(0))))
                    + idMath.Fabs(extents.oGet(2) * ((this.dFar * testAxis.oGet(2).oGet(1)) - (this.dLeft * testAxis.oGet(2).oGet(0))));
            if ((d1 - d2) > 0.0f) {
                return true;
            }
            ...
            // test up/down planes
            d1 = (this.dFar * testOrigin.z) - (this.dUp * testOrigin.x);
            d2 = idMath.Fabs(extents.oGet(0) * ((this.dFar * testAxis.oGet(0).oGet(2)) - (this.dUp * testAxis.oGet(0).oGet(0))))
                    + idMath.Fabs(extents.oGet(1) * ((this.dFar * testAxis.oGet(1).oGet(2)) - (this.dUp * testAxis.oGet(1).oGet(0))))
                    + idMath.Fabs(extents.oGet(2) * ((this.dFar * testAxis.oGet(2).oGet(2)) - (this.dUp * testAxis.oGet(2).oGet(0))));
            if ((d1 - d2) > 0.0f) {
                return true;
            }
        ...
        private boolean CullLocalFrustum(final idFrustum localFrustum, final idVec3[] indexPoints, final idVec3[] cornerVecs) {
            ...
            if (indexPoints[index].x < this.dNear) {
                return true;
            }
            ...
            if (indexPoints[index].x > this.dFar) {
                return true;
            }

            leftScale = this.dLeft * this.invFar;

            // test left plane
            dy = (this.dFar * localFrustum.axis.oGet(1).y) - (this.dLeft * localFrustum.axis.oGet(1).x);
            dz = (this.dFar * localFrustum.axis.oGet(2).y) - (this.dLeft * localFrustum.axis.oGet(2).x);
            index = (FLOATSIGNBITSET(dy) << 1) | FLOATSIGNBITSET(dz);
            dx = (this.dFar * cornerVecs[index].y) - (this.dLeft * cornerVecs[index].x);
            index |= (FLOATSIGNBITSET(dx) << 2);
            ...
            upScale = this.dUp * this.invFar;

            // test up plane
            dy = (this.dFar * localFrustum.axis.oGet(1).z) - (this.dUp * localFrustum.axis.oGet(1).x);
            dz = (this.dFar * localFrustum.axis.oGet(2).z) - (this.dUp * localFrustum.axis.oGet(2).x);
            index = (FLOATSIGNBITSET(dy) << 1) | FLOATSIGNBITSET(dz);
            dx = (this.dFar * cornerVecs[index].z) - (this.dUp * cornerVecs[index].x);
            index |= (FLOATSIGNBITSET(dx) << 2);
        ...
        private boolean LocalLineIntersection(final idVec3 start, final idVec3 end) {
            ...
            // test near plane
            if (this.dNear > 0.0f) {
                d1 = this.dNear - start.x;
                startInside &= FLOATSIGNBITSET(d1);
                if (FLOATNOTZERO(d1)) {
                    d2 = this.dNear - end.x;
                    if ((FLOATSIGNBITSET(d1) ^ FLOATSIGNBITSET(d2)) != 0) {
                        f = d1 / (d1 - d2);
                        if (idMath.Fabs(start.y + (f * dir.y)) <= (this.dNear * leftScale)) {
                            if (idMath.Fabs(start.z + (f * dir.z)) <= (this.dNear * upScale)) {
                                return true;
                            }
                        }
                    }
            ...
            // test far plane
            d1 = start.x - this.dFar;
            startInside &= FLOATSIGNBITSET(d1);
            if (FLOATNOTZERO(d1)) {
                d2 = end.x - this.dFar;
                if ((FLOATSIGNBITSET(d1) ^ FLOATSIGNBITSET(d2)) != 0) {
                    f = d1 / (d1 - d2);
                    if (idMath.Fabs(start.y + (f * dir.y)) <= (this.dFar * leftScale)) {
                        if (idMath.Fabs(start.z + (f * dir.z)) <= (this.dFar * upScale)) {
                            return true;
                        }
                    }
                }
            ...
            fstart = this.dFar * start.y;
            fend = this.dFar * end.y;
            lstart = this.dLeft * start.x;
            lend = this.dLeft * end.x;
            ...
            fstart = this.dFar * start.z;
            fend = this.dFar * end.z;
            lstart = this.dUp * start.x;
            lend = this.dUp * end.x;
        ...
        private void ToClippedPoints(final float[] fractions, idVec3[] points) {
            ...
            scaled.oSet(0, this.axis.oGet(0).oMultiply(this.dFar));
            scaled.oSet(1, this.axis.oGet(1).oMultiply(this.dLeft));
            scaled.oSet(2, this.axis.oGet(2).oMulSet(this.dUp));
            ...
            points[4] = this.origin.oPlus(points[4].oMultiply(fractions[0]));
            points[5] = this.origin.oPlus(points[5].oMultiply(fractions[1]));
            points[6] = this.origin.oPlus(points[6].oMultiply(fractions[2]));
            points[7] = this.origin.oPlus(points[7].oMultiply(fractions[3]));
        }
        ...
        private void ToIndexPointsAndCornerVecs(idVec3[] indexPoints, idVec3[] cornerVecs) {
            ...
            indexPoints[4] = cornerVecs[0].oPlus(this.origin);
            indexPoints[5] = cornerVecs[1].oPlus(this.origin);
            indexPoints[6] = cornerVecs[2].oPlus(this.origin);
            indexPoints[7] = cornerVecs[3].oPlus(this.origin);
        }
        ...
        private void AxisProjection(final idVec3 indexPoints[], final idVec3 cornerVecs[], final idVec3 dir, float[] min, float[] max) {
            ...
            dy = (dir.x * this.axis.oGet(1).x) + (dir.y * this.axis.oGet(1).y) + (dir.z * this.axis.oGet(1).z);
            dz = (dir.x * this.axis.oGet(2).x) + (dir.y * this.axis.oGet(2).y) + (dir.z * this.axis.oGet(2).z);
            index = (FLOATSIGNBITSET(dy) << 1) | FLOATSIGNBITSET(dz);
...
//#endif
            ...
            if (FLOATNOTZERO(d1)) {
                if ((FLOATSIGNBITSET(d1) ^ FLOATSIGNBITSET(d2)) != 0) {
                    ...
                    if (p.x > 0.0f) {
                        ...
                        if (idMath.Fabs(p.z) <= (p.x * upScale)) {
                            p.y = 1.0f;
                            p.z = (p.z * this.dFar) / (p.x * this.dUp);
                            bounds.AddPoint(p);
                        }
                    }
                }
            ...
            if (FLOATNOTZERO(d1)) {
                if ((FLOATSIGNBITSET(d1) ^ FLOATSIGNBITSET(d2)) != 0) {
                    ...
                    if (p.x > 0.0f) {
                        p.y = start.y + (f * dir.y);
                        if (idMath.Fabs(p.y) <= (p.x * leftScale)) {
                            p.y = (p.y * this.dFar) / (p.x * this.dLeft);
                            p.z = 1.0f;
                            bounds.AddPoint(p);
                        }
                    }
            ...
            if ((cull1 == 0) && (start.x > 0.0f)) {
                ...
                p.x = start.x;
                p.y = (start.y * this.dFar) / (start.x * this.dLeft);
                p.z = (start.z * this.dFar) / (start.x * this.dUp);
                bounds.AddPoint(p);
            }
            ...
            if ((cull2 == 0) && (end.x > 0.0f)) {
                ...
                p.x = end.x;
                p.y = (end.y * this.dFar) / (end.x * this.dLeft);
                p.z = (end.z * this.dFar) / (end.x * this.dUp);
                bounds.AddPoint(p);
            }
...
//#endif
        ...
        private void ClipFrustumToBox(final idBox box, float[] clipFractions, int[] clipPlanes) {
            ...
            float f, minf;
            final idMat3 scaled = new idMat3();
			idMat3 localAxis, transpose;
            idVec3 localOrigin;
            final idVec3[] cornerVecs = new idVec3[4];
            final idBounds bounds = new idBounds();

            transpose = box.GetAxis();
            transpose.TransposeSelf();
            localOrigin = (this.origin.oMinus(box.GetCenter())).oMultiply(transpose);
            localAxis = this.axis.oMultiply(transpose);

            scaled.oSet(0, localAxis.oGet(0).oMultiply(this.dFar));
            scaled.oSet(1, localAxis.oGet(1).oMultiply(this.dLeft));
            scaled.oSet(2, localAxis.oGet(2).oMultiply(this.dUp));
            ...
            minf = (this.dNear + 1.0f) * this.invFar;
        ...
        private boolean ClipLine(final idVec3[] localPoints, final idVec3[] points, int startIndex, int endIndex, idVec3 start, idVec3 end, int[] startClip, int[] endClip) {
            ...
            fstart = this.dFar * localStart.y;
            fend = this.dFar * localEnd.y;
            lstart = this.dLeft * localStart.x;
            lend = this.dLeft * localEnd.x;
            ...
            fstart = this.dFar * localStart.z;
            fend = this.dFar * localEnd.z;
            lstart = this.dUp * localStart.x;
            lend = this.dUp * localEnd.x;
