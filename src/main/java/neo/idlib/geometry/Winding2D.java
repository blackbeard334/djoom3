...
//public	final idVec2 	operator[]( final int index ) ;
        ...
        public idVec2 oMinSet(final int index, idVec2 value) {
            return this.p[index].oMinSet(value);
        }

        public idVec2 oPluSet(final int index, idVec2 value) {
            return this.p[index].oPluSet(value);
        }
        ...
        public void Expand(final float d) {
            int i;
            final idVec2[] edgeNormals = new idVec2[MAX_POINTS_ON_WINDING_2D];

            for (i = 0; i < this.numPoints; i++) {
                final idVec2 start = this.p[i];
                final idVec2 end = this.p[(i + 1) % this.numPoints];
            ...
            for (i = 0; i < this.numPoints; i++) {
                this.p[i].oPluSet(edgeNormals[i].oPlus(edgeNormals[((i + this.numPoints) - 1) % this.numPoints]));
            }
        }

        public void ExpandForAxialBox(final idVec2 bounds[]) {
            int i, j, numPlanes;
            final idVec2 v = new idVec2();
            final idVec3[] planes = new idVec3[MAX_POINTS_ON_WINDING_2D];
            idVec3 plane;
			final idVec3 bevel = new idVec3();

            // get planes for the edges and add bevels
            for (numPlanes = i = 0; i < this.numPoints; i++) {
                j = (i + 1) % this.numPoints;
                if ((this.p[j].oMinus(this.p[i])).LengthSqr() < 0.01f) {
                    continue;
                }
                plane = Plane2DFromPoints(this.p[i], this.p[j], true);
                if (i != 0) {
                    if (GetAxialBevel(planes[numPlanes - 1], plane, this.p[i], bevel)) {
            ...
            if (GetAxialBevel(planes[numPlanes - 1], planes[0], this.p[0], bevel)) {
                planes[numPlanes++] = bevel;
            }
            ...
            for (this.numPoints = i = 0; i < numPlanes; i++) {
                if (Plane2DIntersection(planes[((i + numPlanes) - 1) % numPlanes], planes[i], this.p[this.numPoints])) {
                    this.numPoints++;
                }
            }
        }
        ...
        public int Split(final idVec3 plane, final float epsilon, idWinding2D[][] front, idWinding2D[][] back) {
            final float[] dists = new float[MAX_POINTS_ON_WINDING_2D];
            final byte[] sides = new byte[MAX_POINTS_ON_WINDING_2D];
            final int[] counts = new int[3];
            float dot;
            int i, j;
            idVec2 p1, p2;
            final idVec2 mid = new idVec2();
            idWinding2D f;
            idWinding2D b;
            int maxpts;
            ...
            for (i = 0; i < this.numPoints; i++) {
                dists[i] = dot = (plane.x * this.p[i].x) + (plane.y * this.p[i].y) + plane.z;
                if (dot > epsilon) {
                    sides[i] = SIDE_FRONT;
                } else if (dot < -epsilon) {
                    sides[i] = SIDE_BACK;
        ...
        public boolean ClipInPlace(final idVec3 plane, final float epsilon, final boolean keepOn) {
            ...
            final int[] sides = new int[MAX_POINTS_ON_WINDING_2D + 1], counts = new int[3];
            float dot;
            final float[] dists = new float[MAX_POINTS_ON_WINDING_2D + 1];
            idVec2 p1, p2;
			final idVec2 mid = new idVec2();
            final idVec2[] newPoints = new idVec2[MAX_POINTS_ON_WINDING_2D + 4];
...
//	memcpy( p, newPoints, newNumPoints * sizeof(idVec2) );
            System.arraycopy(newPoints, 0, this.p, 0, newNumPoints);
...
//	memcpy( w->p, p, numPoints * sizeof( p[0] ) );
            System.arraycopy(this.p, 0, w.p, 0, this.numPoints);
            return w;
        }
        ...
        public float GetArea() {
            ...
            for (i = 2; i < this.numPoints; i++) {
                d1 = this.p[i - 1].oMinus(this.p[0]);
                d2 = this.p[i].oMinus(this.p[0]);
        ...
        public idVec2 GetCenter() {
            int i;
            final idVec2 center = new idVec2();

            center.Zero();
            for (i = 0; i < this.numPoints; i++) {
                center.oPluSet(this.p[i]);
            }
        ...
        public boolean IsTiny() {
            ...
            edges = 0;
            for (i = 0; i < this.numPoints; i++) {
                delta = this.p[(i + 1) % this.numPoints].oMinus(this.p[i]);
                len = delta.Length();
                if (len > EDGE_LENGTH) {
                    if (++edges == 3) {
                        return false;
...
//		idLib::common->Printf( "(%5.1f, %5.1f)\n", p[i][0], p[i][1] );
        ...
        public float PlaneDistance(final idVec3 plane) {
            ...
            max = -min;
            for (i = 0; i < this.numPoints; i++) {
                d = (plane.x * this.p[i].x) + (plane.y * this.p[i].y) + plane.z;
                if (d < min) {
                    min = d;
...
//                    continue;
        ...
        public boolean PointInside(final idVec2 point, final float epsilon) {
            ...
            for (i = 0; i < this.numPoints; i++) {
                plane = Plane2DFromPoints(this.p[i], this.p[(i + 1) % this.numPoints]);
        ...
        public boolean LineIntersection(final idVec2 start, final idVec2 end) {
            ...
            idVec3 plane;
            final idVec3[] edges = new idVec3[2];

            counts[SIDE_FRONT] = counts[SIDE_BACK] = counts[SIDE_ON] = 0;

            plane = Plane2DFromPoints(start, end);
            for (i = 0; i < this.numPoints; i++) {
                d1 = (plane.x * this.p[i].x) + (plane.y * this.p[i].y) + plane.z;
                if (d1 > epsilon) {
                    sides[i] = SIDE_FRONT;
                } else if (d1 < -epsilon) {
                    sides[i] = SIDE_BACK;
            ...
            for (i = 0; i < this.numPoints; i++) {
                if ((sides[i] != sides[i + 1]) && (sides[i + 1] != SIDE_ON)) {
                    edges[numEdges++] = Plane2DFromPoints(this.p[i], this.p[(i + 1) % this.numPoints]);
                    if (numEdges >= 2) {
                        break;
                    }
                }
        ...
        public boolean RayIntersection(final idVec2 start, final idVec2 dir, float[] scale1, float[] scale2, int[] edgeNums) {
            int i, numEdges;
            final int[] localEdgeNums = new int[2];
            final int[] sides = new int[MAX_POINTS_ON_WINDING_2D + 1], counts = new int[3];
            float d1, d2;
			final float epsilon = 0.1f;
            idVec3 plane;
