        ...
        public boolean UpdateRenderEntity(renderEntity_s renderEntity, final renderView_s renderView) {
            ...
            for (i = 0; i < this.shards.Num(); i++) {
                ...
                for (j = 2; j < winding.GetNumPoints(); j++) {
                    ...
                    tris.indexes.getIntBuffer().put(tris.numIndexes++, tris.numVerts - 3);
                    tris.indexes.getIntBuffer().put(tris.numIndexes++, tris.numVerts - 2);
                    tris.indexes.getIntBuffer().put(tris.numIndexes++, tris.numVerts - 1);
                ...
                for (k = 0; k < this.shards.oGet(i).decals.Num(); k++) {
                    ...
                    for (j = 2; j < decalWinding.GetNumPoints(); j++) {
                        ...
                        decalTris.indexes.getIntBuffer().put(decalTris.numIndexes++, decalTris.numVerts - 3);
                        decalTris.indexes.getIntBuffer().put(decalTris.numIndexes++, decalTris.numVerts - 2);
                        decalTris.indexes.getIntBuffer().put(decalTris.numIndexes++, decalTris.numVerts - 1);
        ...
        private void CreateFractures(final idRenderModel renderModel) {
            ...
            for (i = 0; i < 1 /*renderModel.NumSurfaces()*/; i++) {
                ...
                for (j = 0; j < surf.geometry.numIndexes; j += 3) {
                    w.Clear();
                    for (k = 0; k < 3; k++) {
                        v = surf.geometry.verts[ surf.geometry.indexes.getIntBuffer().get( (j + 2) - k)];
                        w.AddPoint(v.xyz);
