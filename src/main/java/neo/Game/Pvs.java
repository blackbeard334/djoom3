    ...
    public static class idPVS {
        ...
        public idPVS() {
            ...
            for (i = 0; i < MAX_CURRENT_PVS; i++) {
                this.currentPVS[i] = new pvsCurrent_t();
        ...
        public void Init() {
            ...
            this.connectedAreas = new boolean[this.numAreas];
            this.areaQueue = new int[this.numAreas];

            this.areaVisBytes = (((this.numAreas + 31) & ~31) >> 3);
            this.areaVisLongs = this.areaVisBytes / Long.BYTES;

            this.areaPVS = new byte[this.numAreas * this.areaVisBytes];//	memset( areaPVS, 0xFF, numAreas * areaVisBytes );
            Arrays.fill(this.areaPVS, 0, this.numAreas * this.areaVisBytes, (byte) 0xFF);

            this.numPortals = GetPortalCount();

            this.portalVisBytes = (((this.numPortals + 31) & ~31) >> 3);
            this.portalVisLongs = this.portalVisBytes / Long.BYTES;

            for (int i = 0; i < MAX_CURRENT_PVS; i++) {
                this.currentPVS[i].handle.i = -1;
                this.currentPVS[i].handle.h = 0;
                this.currentPVS[i].pvs = new byte[this.areaVisBytes];//memset( currentPVS[i].pvs, 0, areaVisBytes );
            }
        ...
        public void Shutdown() {
            if (this.connectedAreas != null) {
//		delete connectedAreas;
                this.connectedAreas = null;
            }
            if (this.areaQueue != null) {
//		delete areaQueue;
                this.areaQueue = null;
            }
            if (this.areaPVS != null) {
//		delete areaPVS;
                this.areaPVS = null;
            }
            if (this.currentPVS != null) {
                for (int i = 0; i < MAX_CURRENT_PVS; i++) {
//			delete currentPVS[i].pvs;
        ...
        public pvsHandle_t SetupCurrentPVS(final idBounds source, final pvsType_t type /*= PVS_NORMAL*/) {
            int numSourceAreas;
            final int[] sourceAreas = new int[MAX_BOUNDS_AREAS];
...
//		memset( currentPVS[handle.i].pvs, 0, areaVisBytes );
                Arrays.fill(this.currentPVS[handle.i].pvs, 0, this.areaVisBytes, (byte) 0);
                return handle;
            }
...
//	memset( connectedAreas, 0, numAreas * sizeof( *connectedAreas ) );
            Arrays.fill(this.connectedAreas, 0, this.numAreas, false);
        ...
        public pvsHandle_t SetupCurrentPVS(final int[] sourceAreas, final int numSourceAreas, final pvsType_t type /*= PVS_NORMAL*/) {
            ...
            if ((0 == numSourceAreas) || (sourceAreas[0] < 0) || (sourceAreas[0] >= this.numAreas)) {
                Arrays.fill(this.currentPVS[handle.i].pvs, 0, this.areaVisBytes, (byte) 0);//memset(currentPVS[handle.i].pvs, 0, areaVisBytes);
                return handle;
            }
            ...
            if (type != PVS_CONNECTED_AREAS) {
                ...
                for (i = 1; i < numSourceAreas; i++) {
                    ...
                    final int vOffset = (sourceAreas[i] * this.areaVisBytes) / Long.BYTES;
                    vis = reinterpret_cast_long_array(this.areaPVS);
                    pvs = reinterpret_cast_long_array(this.currentPVS[handle.i].pvs);
                    for (j = 0; j < this.areaVisLongs; j++) {
                        pvs[j] |= vis[j + vOffset];
                    }
                }
            } else {
                Arrays.fill(this.currentPVS[handle.i].pvs, 0, this.areaVisBytes, (byte) -1);//memset( currentPVS[handle.i].pvs, -1, areaVisBytes );
            }
            ...
            Arrays.fill(this.connectedAreas, 0, this.numAreas, false);//memset( connectedAreas, 0, numAreas * sizeof( *connectedAreas ) );
        ...
        public pvsHandle_t MergeCurrentPVS(pvsHandle_t pvs1, pvsHandle_t pvs2) {
            ...
            ptr = reinterpret_cast_long_array(this.currentPVS[handle.i].pvs);
            pvs1Ptr = reinterpret_cast_long_array(this.currentPVS[pvs1.i].pvs);
            pvs2Ptr = reinterpret_cast_long_array(this.currentPVS[pvs2.i].pvs);
        ...
        public boolean InCurrentPVS(final pvsHandle_t handle, final idBounds target) {
            ...
            final int[] targetAreas = new int[MAX_BOUNDS_AREAS];
            ...
            for (i = 0; i < numTargetAreas; i++) {
                if ((this.currentPVS[handle.i].pvs[targetAreas[i] >> 3] & (1 << (targetAreas[i] & 7))) != 0) {
                    return true;
                }
            }
            return false;
        ...
        public void DrawPVS(final idVec3 source, final pvsType_t type /*= PVS_NORMAL*/) {
            ...
            for (j = 0; j < this.numAreas; j++) {
                ...
                if (0 == (this.currentPVS[handle.i].pvs[j >> 3] & (1 << (j & 7)))) {
                    continue;
                }
        ...
        public void DrawPVS(final idBounds source, final pvsType_t type /*= PVS_NORMAL*/) {
            ...
            final int[] areas = new int[MAX_BOUNDS_AREAS];
            exitPortal_t portal;
            final idPlane plane = new idPlane();
            idVec3 offset;
            idVec4 color;
...
// #if ASYNC_WRITE_PVS
        public void WritePVS(final pvsHandle_t handle, idBitMsg msg) {
            msg.WriteData(ByteBuffer.wrap(this.currentPVS[handle.i].pvs), this.areaVisBytes);
        }

        public void ReadPVS(final pvsHandle_t handle, final idBitMsg msg) {
            final ByteBuffer l_pvs = ByteBuffer.allocate(256);
            int i;

            assert (this.areaVisBytes <= 256);
            msg.ReadData(l_pvs, this.areaVisBytes);
            if (memcmp(l_pvs.array(), this.currentPVS[handle.i].pvs, this.areaVisBytes)) {
...
//	memset( pvsAreas, 0, numAreas * sizeof( *pvsAreas ) );
            ...
            for (i = 0; i < this.numAreas; i++) {
                ...
                area = this.pvsAreas[i] = new pvsArea_t();
                area.bounds.Clear();
//                area.portals = portalPtrs + cp;
                ...
                for (j = 0; j < n; j++) {
                    ...
                    p = this.pvsPortals[cp++] = new pvsPortal_t();
...
//		memcpy( p.mightSee, p.vis, portalVisBytes );
        ...
        private void FloodFrontPortalPVS_r(pvsPortal_t portal, int areaNum) {
            ...
            for (i = 0; i < area.numPortals; i++) {
                p = area.portals[i];
                n = indexOf(p, this.pvsPortals);//TODO:very importante, what does thus do!?
...
//		stack.mightSee = (reinterpret_cast<byte *>(stack)) + sizeof(pvsStack_t);TODO:check this..very importante
                stack.mightSee = new byte[this.portalVisBytes];
                stack.next = null;
...
//		delete[] s;
        ...
        private void CreatePassages() {
            ...
            final int[] numBounds = new int[1];
            final int[] sides = new int[MAX_PASSAGE_BOUNDS];
            final idPlane[] passageBounds = new idPlane[MAX_PASSAGE_BOUNDS];
            ...
            for (i = 0; i < this.numPortals; i++) {
                ...
                for (j = 0; j < area.numPortals; j++) {
                    ...
                    n = indexOf(target, this.pvsPortals);
...
//		delete[] p.passages;
        ...
        private int AreaPVSFromPortalPVS() {
            ...
            Arrays.fill(this.areaPVS, 0, this.numAreas * this.areaVisBytes, (byte) 0);

            for (i = 0; i < this.numAreas; i++) {
                area = this.pvsAreas[i];
//                pvs = areaPVS + i * areaVisBytes;
                pvs = i * this.areaVisBytes;

                // the area is visible to itself
                this.areaPVS[pvs + (i >> 3)] |= 1 << (i & 7);
                ...
                for (j = 0; j < area.numPortals; j++) {
                    k = indexOf(area.portals[j], this.pvsPortals);
                ...
                for (j = 0; j < this.numPortals; j++) {
                    ...
                    if ((portalPVS[j >> 3] & (1 << (j & 7))) != 0) {
                        ...
                        this.areaPVS[pvs + (areaNum >> 3)] |= 1 << (areaNum & 7);
                    }
                }

                // count the number of visible areas
                for (j = 0; j < this.numAreas; j++) {
                    if ((this.areaPVS[pvs + (j >> 3)] & (1 << (j & 7))) != 0) {
                        totalVisibleAreas++;
                    }
                }
            }
        ...
        private pvsHandle_t AllocCurrentPVS( /*unsigned*/int h) {
            int i;
            final pvsHandle_t handle = new pvsHandle_t();
